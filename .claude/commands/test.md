...
Run all Pester tests for the GA-AppLocker GUI project and explain any failures.
...
3. Run all tests in the GUI/Tests folder:
```powershell
Invoke-Pester -Path ./GUI/Tests -Output Detailed -PassThru
```
...
```powershell
Invoke-Pester -Path ./GUI/Tests -Output Detailed -CodeCoverage ./GUI/*.ps1,./GUI/*.psm1
```
