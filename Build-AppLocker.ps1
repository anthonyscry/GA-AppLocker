...
function Invoke-Validate {
    ...
    $xamlValidatorPath = Join-Path $ProjectRoot 'GUI\Scripts\Test-XamlValidation.ps1'
    if (Test-Path $xamlValidatorPath) {
        try {
            $xamlResults = & $xamlValidatorPath -Path (Join-Path $ProjectRoot 'GUI') 2>&1
    ...
    $modulePath = Join-Path $ProjectRoot 'utilities\Common.psm1'
    ...
    $asyncHelpersPath = Join-Path $ProjectRoot 'GUI\AsyncHelpers.psm1'
...
function Invoke-Test {
    ...
    if (-not $CI) {
        ...
        $pesterConfig.CodeCoverage.Path = @(
            (Join-Path $ProjectRoot 'GUI\AsyncHelpers.psm1'),
            (Join-Path $ProjectRoot 'utilities\Common.psm1')
...
function Invoke-Package {
    ...
    # Copy directories
    $includeDirs = @(
        @{ Source = 'utilities'; Exclude = @('*.Tests.ps1') },
        @{ Source = 'GUI'; Exclude = @('*.Tests.ps1', 'Scripts', 'Tests') },
        @{ Source = 'ADManagement'; Exclude = @() }
    )
    ...
    foreach ($dir in $includeDirs) {
        ...
        if (Test-Path $sourceDir) {
            ...
            $destDir = Join-Path $packagePath $dir.Source
