    ...
    try {
        ...
        if ($RuleType -eq 'Hash' -or $RuleType -eq 'All') {
            ...
            foreach ($group in ($hashGroups | Where-Object { $_.Count -gt 1 })) {
                ...
                $toRemove += $duplicates
                $result.KeptRules += [PSCustomObject]@{
                    Id = $keep.Id
                    Name = $keep.Name
                    Type = 'Hash'
                    Key = $keep.Hash
        ...
        foreach ($rule in $toRemove) {
            ...
            try {
                if (Test-Path $rule._FilePath) {
                    Remove-Item -Path $rule._FilePath -Force
                    $result.RemovedCount++
                    $result.RemovedRules += [PSCustomObject]@{
                        Id = $rule.Id
                        Name = $rule.Name
                        Type = $rule.RuleType
                    }
    ...
    try {
        ...
        if ($RuleType -eq 'Hash' -or $RuleType -eq 'All') {
            ...
            foreach ($group in $hashGroups) {
                ...
                $result.DuplicateGroups += [PSCustomObject]@{
                    Type = 'Hash'
...
.DESCRIPTION
    ...
    Used by New-HashRule to prevent creating duplicates.
...
.DESCRIPTION
    ...
    Used by New-PublisherRule to prevent creating duplicates.
