...
.EXAMPLE
    .\scripts\Invoke-LocalValidation.ps1 -Fix
    # Attempts to auto-fix issues
#>
...
try {
    Write-Host "`nGA-AppLocker Local Validation" -ForegroundColor $ColorInfo
    ...
    if (-not $analyzer) {
        ...
        # Use root settings if available, otherwise GUI settings
        $settingsPath = Join-Path $repoRoot '.PSScriptAnalyzerSettings.psd1'
        if (-not (Test-Path $settingsPath)) {
            $settingsPath = Join-Path $repoRoot 'GUI\PSScriptAnalyzerSettings.psd1'
        }
    ...
    Write-Header "XAML Validation"

    $xamlFiles = Get-ChildItem -Path $repoRoot -Include '*.xaml' -Recurse -File |
        Where-Object { $_.FullName -notmatch '[\\/](\.git|bin|obj)[\\/]' }

    if ($xamlFiles.Count -eq 0) {
        Write-Result 'SKIP' 'No XAML files found'
    } else {
        Write-Host "Validating $($xamlFiles.Count) XAML files..." -ForegroundColor $ColorMuted
        ...
        if ($wpfAvailable) {
            foreach ($file in $xamlFiles) {
                ...
                try {
                    ...
                    Write-Result 'FAIL' $relativePath
